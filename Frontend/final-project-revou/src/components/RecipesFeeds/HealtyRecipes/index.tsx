      ...
      <div className="">
        {chefMainCard2[0].recipe.map((recipeCategory, index) => (
          <div className="grid grid-cols-5 gap-2 px-10 pt-5" key={index}>
            {showAll
              ? recipeCategory.category.Healtyrecipes.map(
                  (recipe, recipeIndex) => (
                    <div key={recipeIndex}>
              ...
              : recipeCategory.category.Healtyrecipes.slice(0, 5).map(
                  (recipe, recipeIndex) => (
                    <div key={recipeIndex}>
                      <img src={recipe.image} alt="" className="rounded-xl" />
                    </div>
        ...
        {chefMainCard2[0].recipe.some(
          (category) => category.category.Healtyrecipes.length > 5
        ) && (
