...
const MainDishes_vmhb: React.FC = () => {
      ...
      <div className=" pr-16 ">
        {chefMainCard2[0].recipe.map((recipeCategory, index) => (
          <div className="grid grid-cols-4 gap-4 px-10  pt-4 " key={index}>
            {recipeCategory.category.MainDishes.slice(0, showCount).map(
              (recipe, recipeIndex) => (
                <div
                  key={recipeIndex}
        ...
        <div className="px-10 pt-4 flex justify-end items-center gap-2">
          ...
          {showCount <
            chefMainCard2[0].recipe[0].category.MainDishes.length && (
...
export default MainDishes_vmhb;
